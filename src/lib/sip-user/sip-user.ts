...
export class SipUser {
  ...
  private held = false;
  private options: SipUserOptions;
  private registerer: Registerer | undefined = undefined;
  private registerRequested = false;
  private userAgent: UserAgent;

  /**
   * Constructs a new instance of the `SipUser` class.
   * @param server - SIP WebSocket Server URL.
   * @param options - Options bucket. See {@link SipUserOptions} for details.
   */
  constructor(server: string, options: SipUserOptions = {}) {
    // Delegate
    ...
    this.userAgent.delegate = {
      ...
      onInvite: (invitation: Invitation): void => {
        ...
        if (this.delegate && this.delegate.onCallReceived) {
          this.delegate.onCallReceived(invitation.request);
        } else {
    ...
    this.logger = this.userAgent.getLogger('sip.react-sip');
      ...
      this.registerer.stateChange.addListener((state: RegistererState) => {
        switch (state) {
          case RegistererState.Initial:
            if (this.delegate && this.delegate.onRegistering) {
              this.delegate.onRegistering();
            }
            break;
          ...
          case RegistererState.Terminated:
            ...
            break;
          default:
            if (this.delegate && this.delegate.onRegisterFailed) {
              // @ts-ignore
              this.delegate.onRegisterFailed(this.registerer?.stateError || undefined);
            }
        }
      });
    }
  ...
  public call(
    ...
    if (this.delegate && this.delegate.onCallOutgoing) {
      this.delegate.onCallOutgoing(inviter)
    }
